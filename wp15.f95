...
! test function M(zr)
speed_zr = M(zr)
write(*,*) " M(zr) returned ",speed_zr," for zr (m) = ",zr
!print'("M(zr) = ",*," m/s")',speed_zr,'("At a height: ",F7.2," m.")',zr
...
! with physically unrealtistic negative values.
!
...
! zr is greater than maximum height in sounding: M = -12000.0
  ...
  if (zr .lt. min_height) then 
    M = -5.0
  else if (zr .gt. max_height) then
    M = -20.0
  endif

  ! convery hub height zr to MSL using first entry in sounding file
  ...
  if (itop .gt. 1) then
    ...
    M = speed_array(itop - 1) + ratio*(speed_array(itop) - speed_array(itop - 1))
  else
    ratio = zrmsl/zmsl(itop)  ! Assumption is z at bottom = 0
    M = ratio*speed_array(itop) ! Boundary condition is speed at 0 = 0
  endif

!   write(*,*) "********************"
!   write(*,*) " In function M(zr) and zr = ",zr
!   write(*,*) " In function M(zr) and M = ",M
!   write(*,*) " and min_height = ",min_height," at array location ",min_loc
!   write(*,*) " and max_height = ",max_height
!   write(*,*) "********************"
...
subroutine findpower
  ...
  do i = 20,1,-1
    zz = zref + real(i)*delz
    speedz = M(zz)
    write(*,*) "*****In subroutine findpower*****"
    write(*,*) "zz (m AGL), M (m/s) = ",zz, speedz
  enddo
